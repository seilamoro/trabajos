...
<?PHP
    ...
    for ($i = 0; $i < COUNT($_POST['estancias']); $i++) {
		...
		$estancia_agrupar = SPLIT('\*',$_POST['estancias'][$i]);
	...
	echo 'function recargar_datos() {';
		for ($i = 0; $i < COUNT($_POST['estancias']); $i++) {
			echo 'if (formulario.dni_factura_select.value == "'.$_SESSION['estancias_agrupar'][$i]['fila']['DNI_Cl'].'") {';
				echo 'formulario.nombre_factura.value = "'.$_SESSION['estancias_agrupar'][$i]['fila']['Nombre_Cl'].'";';
				echo 'formulario.apellido1_factura.value = "'.$_SESSION['estancias_agrupar'][$i]['fila']['Apellido1_Cl'].'";';
				echo 'formulario.apellido2_factura.value = "'.$_SESSION['estancias_agrupar'][$i]['fila']['Apellido2_Cl'].'";';
			echo '}';
		}
	echo '}';
	...
	<tr id="titulo_tablas">
		<td align="left" style="padding:0px 0px 0px 0px;" width="100%">
			...
			<div class="champi_centro" style="width:<?PHP echo $_SESSION['gdh']['tamanios']['gdh_distribucion_ancho']; ?>;">
					Factura Agrupada de Alberguistas
			</div>
	...
	for ($i = 0; $i < COUNT($_SESSION['estancias_agrupar']); $i++) {
		...
		if (isset($_SESSION['estancias_agrupar'][$i]['fila']['Id_Servicios'])) {
			...
			$servicio = MYSQL_FETCH_ARRAY(MYSQL_QUERY($sql));
		  	...
		  	$_SESSION['estancias_agrupar'][$i]['fila']['precio_servicio'] = $servicio['Precio'];
		}
		else {
		  	$_SESSION['estancias_agrupar'][$i]['fila']['precio_servicio'] = 0;
		  	$plus = 0;
		}
				
		$_SESSION['estancias_agrupar'][$i]['resto'] = FLOATVAL(($_SESSION['estancias_agrupar'][$i]['tarifa']['Tarifa'] + $plus) * $_SESSION['estancias_agrupar'][$i]['fila']['PerNocta'] - $_SESSION['estancias_agrupar'][$i]['fila']['Ingreso']);
		$resto = $resto + $_SESSION['estancias_agrupar'][$i]['resto'];
	}
...
?>
											  		...
											  		<td align="right">
		  												<select class="detalles_select" name="exento__<?PHP echo $_SESSION['estancias_agrupar'][$i]['fila']['DNI_Cl'].'__'.STR_REPLACE('-','_',$_SESSION['estancias_agrupar'][$i]['fila']['Fecha_Llegada']); ?>" onchange="modificar_resto_factura_exentos(resto_factura,total_importe_factura,exento__<?PHP echo $_SESSION['estancias_agrupar'][$i]['fila']['DNI_Cl'].'__'.STR_REPLACE('-','_',$_SESSION['estancias_agrupar'][$i]['fila']['Fecha_Llegada']); ?>,<?PHP echo $_SESSION['estancias_agrupar'][$i]['fila']['Ingreso']; ?>,<?PHP echo FLOATVAL($_SESSION['estancias_agrupar'][$i]['tarifa']['Tarifa'] * $_SESSION['estancias_agrupar'][$i]['fila']['PerNocta'] - $_SESSION['estancias_agrupar'][$i]['fila']['Ingreso']); ?>,cuantia_desperfecto_factura);">
...
?>
				...
				<tr>
					...
					<td align="left">
						<input type="radio" name="escoger_dni" value="1" checked onclick="desactivar_campos(dni_factura_select,dni_factura_text,1); recargar_datos();">
						<select name="dni_factura_select" class="detalles_select" editable="true" editText="Editable Text" onchange="recargar_datos();">
<?PHP
	for ($i = 0; $i < COUNT($_SESSION['estancias_agrupar']); $i++) {
?>
							<option value="<?PHP echo $_SESSION['estancias_agrupar'][$i]['fila']['DNI_Cl']?>"><?PHP echo $_SESSION['estancias_agrupar'][$i]['fila']['DNI_Cl']?>
<?PHP
	}
?>
						</select>
				...
				<tr>
					...
					<td align="left">
						<input type="radio" name="escoger_dni" value="0" onclick="desactivar_campos(dni_factura_text,dni_factura_select,0);">
						<input type="text" name="dni_factura_text" size=15 maxlength=30 contenteditable=true class="input_text_vacio" value="" disabled="true">
					</td>
				</tr>
				<tr>
					<td class="texto_detalles" align="left">
...
?>
						<input type="text" name="cuantia_desperfecto_factura" size=6 maxlength=7 contenteditable=true class="input_text_vacio_numerico" value="<?PHP //echo $_POST['cuantia_desperfecto_factura']; ?>" onkeyup="modificar_resto_factura_desperfecto(resto_factura,total_importe_factura,cuantia_desperfecto_factura);">
<?PHP
	}
	else {
?>
						<input type="text" name="cuantia_desperfecto_factura" size=6 maxlength=7 contenteditable=true class="input_text_vacio_numerico" value="0.0" onkeyup="modificar_resto_factura_desperfecto(resto_factura,total_importe_factura,cuantia_desperfecto_factura);">
<?PHP
	}
?>
					</td>												
				...
				<tr>
					...
					<td align="left">
						<input type="hidden" name="resto_factura" value="<?PHP echo $resto.' €'; ?>">
