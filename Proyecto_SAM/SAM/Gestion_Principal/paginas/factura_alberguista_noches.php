<script language="javascript">
	...
	// Función que cambia el valor del resto de la factura que queda por pagar.
	function cambiar_resto_factura(seleccion,campo_resto,campo_total,resto,ingreso,desperfecto) {
	  	if (seleccion.value == 'N') {
	  	  	if (isNaN(parseFloat(desperfecto.value))) {
				campo_resto.value = resto + ' €';
		  	...
		  	if (isNaN(parseFloat(desperfecto.value))) {
				campo_resto.value = (-1 * ingreso) + ' €';
				campo_total.value = (-1 * ingreso) + ' €';
			}
			else {
			  	campo_resto.value = (-1 * ingreso) + ' €';
			  	campo_total.value = ((-1 * ingreso) + parseFloat(desperfecto.value)) + ' €';
			}
		}
	}
...
<?PHP	
		...
		$resto = 0;
		//$resto = FLOATVAL(($tarifa['Tarifa'] + $plus) * $fila['PerNocta'] - $fila['Ingreso']);
?>
				<tr>
					<td class="texto_detalles" align="left">
						Exento:
					...
					<td align="left" colspan="3">
						<select name="exento_factura" class="seleccion" onchange="cambiar_resto_factura(exento_factura,resto_factura,total_importe_factura,<?PHP echo $resto; ?>,<?PHP echo $fila['Ingreso']; ?>,cuantia_desperfecto_factura);">
				...
				<tr>
					...
					<td align="left">
						<input type="text" name="cuantia_desperfecto_factura" size=6 maxlength=7 contenteditable=true class="input_text_vacio_numerico" value="0.0" onkeyup="cambiar_resto_factura(exento_factura,resto_factura,total_importe_factura,<?PHP echo $resto; ?>,<?PHP echo $fila['Ingreso']; ?>,cuantia_desperfecto_factura);">
					</td>												
				</tr>
				<tr>
					<td class="texto_detalles" align="left">
				...
				<tr>
					...
					<td align="left">
						<input type="text" name="pagadas_factura" size=9 contenteditable=false class="input_text" value="<?PHP echo $fila['Noches_Pagadas']." (de ".$fila['PerNocta'].")"; ?>">
					...
					<td colspan="4" align="center">
						<table>
							<tr>
								...
								<td class="texto_detalles">
									<img src="./../imagenes/botones-texto/facturar.jpg" class="imagen-boton" title="Crear Nueva Factura y Dividir la Estancia" value="Facturar" onclick="comprueba_dni('AN');">
<script language="javascript">
