	...
	function llevar() {
		...
		for (i=0;i<document.getElementsByName('op').length;i++){
			if (document.getElementsByName('op')[i].checked == true)  {
				...
				window.opener.document.ingresar_estancia.dni_componente.value = array_cadena[0];
				
				window.opener.document.ingresar_estancia.tipo_doc_componente.value = array_cadena[1];

				//window.opener.document.dar_alta.nacionalidad.options[array_cadena[2]-1].selected = true;
					var nacionalidad = array_cadena[2]-1;
					for(j=0;j<window.opener.document.ingresar_estancia.nacionalidad_componente.length;j++)	{
						if(j == nacionalidad)	{
							window.opener.document.ingresar_estancia.nacionalidad_componente.options[j].selected = true;
							break;
						}
		...
		/*	window.opener.document.ingresar_estancia.diaEx_componente.options[parseFloat(array_cadena[5])].selected = true;
                  window.opener.document.ingresar_estancia.mesEx_componente.options[parseFloat(array_cadena[4])].selected = true;
				//window.opener.document.ingresar_estancia.annoEx_componente.options[array_cadena[3]-1900+1].selected = true;
			 for(j=0;j<window.opener.document.ingresar_estancia.annoEx_componente.length;j++)	{
						if(window.opener.document.ingresar_estancia.annoEx_componente.options[j].value == array_cadena[3])	{
							window.opener.document.ingresar_estancia.annoEx_componente.options[j].selected = true;
							break;
						}
						
					}
		
				window.opener.document.ingresar_estancia.diaNa_componente.options[parseFloat(array_cadena[8])].selected = true;
				//window.opener.document.ingresar_estancia.annoNa_componente.options[array_cadena[6]-1900+1].selected = true;
				 for(j=0;j<window.opener.document.ingresar_estancia.annoNa_componente.length;j++)	{
						if(window.opener.document.ingresar_estancia.annoNa_componente.options[j].value == array_cadena[6])	{
							window.opener.document.ingresar_estancia.annoNa_componente.options[j].selected = true;
							break;
						}
						
					}
				window.opener.document.ingresar_estancia.mesNa_componente.options[parseFloat(array_cadena[7])].selected = true;*/
				window.opener.document.ingresar_estancia.nombre_componente.value = array_cadena[9];
				window.opener.document.ingresar_estancia.ape1_componente.value = array_cadena[10];
				window.opener.document.ingresar_estancia.ape2_componente.value = array_cadena[11];
				
				if(array_cadena[12]=="F"){
				window.opener.document.ingresar_estancia.sexo[0].checked=true ;
				}else if(array_cadena[12]=="M"){
				window.opener.document.ingresar_estancia.sexo[1].checked=true ;
				}
...
<center>
<br><br><br>
<form name="formu" id="formu" action="pop_grupos_componentes_reserva.php" method="POST">
...
if($_GET['grupo_mo']){
	...
	<table border="0" width="98%" align="center">
		...
		<tr align="left" >
			...
			<td><input type="criterio_dni" name="criterio_dni" size="11"  maxlength="30" style="border:solid 1px;" onKeyUp="enviar();" onFocus="caja('dni');" value="<?
								if(isset($_POST['criterio_dni'])){
									echo $_POST['criterio_dni'];
								}?>"></td>
			<td><input type="criterio_nombre" name="criterio_nombre" size="13" maxlength="20" style="border:solid 1px;" onKeyUp="enviar();" onFocus="caja('nombre');" value="<?
								if(isset($_POST['criterio_nombre'])){
									echo $_POST['criterio_nombre'];
								}?>"></td>
			<td><input type="criterio_ape1" name="criterio_ape1" size="15" smaxlength="30" style="border:solid 1px;" onKeyUp="enviar();" onFocus="caja('ape1');" value="<?
								if(isset($_POST['criterio_ape1'])){
									echo $_POST['criterio_ape1'];
								}?>"></td>
			<td><input type="criterio_ape2" name="criterio_ape2" size="17" maxlength="30" style="border:solid 1px;" onKeyUp="enviar();" onFocus="caja('ape2');" value="<?
	...
	<?
		@ $db = mysql_pconnect($_SESSION['conexion']['host'],$_SESSION['conexion']['user'],$_SESSION['conexion']['pass']);
			...
			$where = " WHERE DNI_CL LIKE '".$_POST['criterio_dni']."%' and Nombre_CL LIKE '".$_POST['criterio_nombre']."%'  and Apellido1_CL LIKE '".$_POST['criterio_ape1']."%' and Apellido2_CL LIKE '".$_POST['criterio_ape2']."%'";
			...
			?>
			<div class="tableContainer" style="height:410px;width:570px;">
...
$result2=mysql_query($sql);
		while ($fila2=mysql_fetch_array($result2)) {
				...
				$result_grupo=mysql_query("select * from estancia_gr,componentes_grupo where estancia_gr.Fecha_Llegada=componentes_grupo.Fecha_Llegada and estancia_gr.Nombre_Gr=componentes_grupo.Nombre_Gr and componentes_grupo.DNI='".$row2['DNI_Cl']."' and estancia_gr.Fecha_Llegada<='".$fecha_llegada."' and Fecha_Salida>'".$fecha_llegada."'");
