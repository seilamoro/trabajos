<?
	...
	if($result_mod = mysql_query($sql_mod)){
		//Comprobamos que haya devuelto algún registro
		if(mysql_num_rows($result_mod) > 0){
			$fila_mod=  mysql_fetch_array($result_mod);
			//Dividimos la fecha de nacimiento en dia, mes y año para facilitar su uso
			$fecha_nacimiento = split("-",$fila_mod['Fecha_Nacimiento_Cl']);	
?>
		<!--- MODIFICACIÓN ALBERGUISTA -->
			...
			<input type="hidden" name="dni_ant" value="<?echo $fila_mod['DNI_Cl']?>">
				...
				<td align="left" colspan="2">
					<table> 
						...
						<SELECT class='select_formulario'  name="dia_fecha_expedicion">
							<?
							//Guardo en el array $fecha_mod la fecha separada en dia ($fecha_mod[2]), mes ($fecha_mod[1])  y año ($fecha_mod[0])
				...
				<tr rowspan="2">
					<td colspan="3" align="center" >
						...
						?>" >
							<a href="#" onClick="if(valida_campos(0)){document.forms[0].submit();}"><img src="../imagenes/botones-texto/modificar.jpg" alt="Realizar Modificación de Alberguista"  border="0"/></a>	&nbsp;&nbsp;
							<?
							if(isset($_GET['busqueda']) && $_GET['busqueda']=="si"){
							?>
							<a href="?pag=listado_criterio_alberguistas.php"><img src="../imagenes/botones-texto/cancelar.jpg" alt="Cancelar Modificación de Alberguista" border="0"/></a>	
							<?
							}else{
							?>
							<a href="?pag=alberguistas.php"><img src="../imagenes/botones-texto/cancelar.jpg" alt="Cancelar Modificación de Alberguista" border="0"/></a>	
							<?
							}	
							?>
						</div>
...
<?
		...
		//Mensaje de error y redirección a la página de búsquedas si ha accedido desde ella, o a alberguistas.php en caso contrario
