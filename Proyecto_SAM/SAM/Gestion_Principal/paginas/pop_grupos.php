	...
	function llevar() {
		...
		for (i=0;i<document.getElementsByName('op').length;i++){
			if (document.getElementsByName('op')[i].checked == true)  {
				...
				window.opener.document.dar_alta.dni_alta.value = array_cadena[0];
				
				if(array_cadena[1]=="")
					array_cadena[1]="D";
				window.opener.document.dar_alta.tipo_doc_alta.value = array_cadena[1];
var meses = new Array ("","Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre");
				//window.opener.document.dar_alta.nacionalidad.options[array_cadena[2]-1].selected = true;
					...
					for(j=0;j<window.opener.document.dar_alta.nacionalidad.length;j++)	{
						if(j == nacionalidad)	{
							window.opener.document.dar_alta.nacionalidad.options[j].selected = true;
							break;
						}
			...
			//window.opener.document.dar_alta.diaEx.options[parseFloat(array_cadena[5])].selected = true;
                   //window.opener.document.dar_alta.mesEx.options[parseFloat(array_cadena[4])].selected = true;
				//window.opener.document.dar_alta.annoEx.options[array_cadena[3]-1900+1].selected = true;
			/*for(j=0;j<window.opener.document.dar_alta.annoEx.length;j++)	{
						if(window.opener.document.dar_alta.annoEx.options[j].value == array_cadena[3])	{
							window.opener.document.dar_alta.annoEx.options[j].selected = true;
							break;
						}
			...
			/*	window.opener.document.dar_alta.diaNa.options[parseFloat(array_cadena[8])].selected = true;
				
						
						for(j=0;j<window.opener.document.dar_alta.annoNa.length;j++)	{
						if(window.opener.document.dar_alta.annoNa.options[j].value == array_cadena[6])	{
							window.opener.document.dar_alta.annoNa.options[j].selected = true;
							break;
						}
						
					}
				//window.opener.document.dar_alta.annoNa.options[array_cadena[6]+1].selected = true;
				window.opener.document.dar_alta.mesNa.options[parseFloat(array_cadena[7])].selected = true;*/
				
				window.opener.document.dar_alta.nombreR.value = array_cadena[9];
				window.opener.document.dar_alta.ape1.value = array_cadena[10];
				window.opener.document.dar_alta.ape2.value = array_cadena[11];
				window.opener.document.dar_alta.telefono.value = array_cadena[12];
				
				if(array_cadena[13]=="F")
					window.opener.document.dar_alta.sexo_r[0].checked = true;
				else if(array_cadena[13]=="M")
					window.opener.document.dar_alta.sexo_r[1].checked = true;
				window.close();
			}
		}
	}
...
<form name="formu" id="formu" action="pop_grupos.php" method="POST">
	...
	return $nom_partido;
		...
		@ $db = mysql_pconnect($_SESSION['conexion']['host'],$_SESSION['conexion']['user'],$_SESSION['conexion']['pass']);
			...
			$where = " WHERE Fecha_Salida<='".date("Y-m-d")."' and DNI_Repres LIKE '".$_POST['criterio_dni']."%' and Nombre_Repres  LIKE '".$_POST['criterio_nombre']."%'  and Apellido1_Repres LIKE '".$_POST['criterio_ape1']."%' and Apellido2_Repres LIKE '".$_POST['criterio_ape2']."%'";
			
			$result=mysql_query("select DISTINCT * from estancia_gr $where GROUP BY dni_repres ");
			
			//$where2 = " WHERE DNI_CL LIKE '".$texto."%' and Nombre_CL LIKE '".$texto_nombre."%'  and Apellido1_CL LIKE '".$texto_ape1."%' and Apellido2_CL LIKE '".$texto_ape2."%'";
...
?>
				...
				if (mysql_num_rows($result_incidencia) != 0) {
					?>
					<td width="28"><img src="../../imagenes/botones/alerta.gif" alt="Ha provocado alguna incidencia con anterioridad" height="25px;" width="25px;" title="Cargar el representante" /></td>
					<? 
				} else {
					?>
					<td width="24">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
	        ...
	        <a href="#">
			<img src="../../imagenes/botones-texto/aceptar.jpg"  name="boton_aceptar" value="aceptar" onClick="llevar();" border="0">
			</a>
			</div>
		</form>
		</center>
