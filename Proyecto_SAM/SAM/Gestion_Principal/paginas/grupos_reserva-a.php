	...
	function mover_fecha(dia,llegada,salida,valor){
			...
			if(mod != "si" && nume != "0"){ // si se está modificando o no se ha encontrado la solicitud, no se deja asignar camas
				...
				if(document.getElementById(n).value != "0"){ // si el valor del hidden es distinto a cero es que se esta desasignando
					...
					if(exi){
						...
						if(cambio)		
							quitar_cambio_hab(fecha_tem.substring(8,10)+"/"+fecha_tem.substring(5,7)+"/"+fecha_tem.substring(0,4));
					}
					...
					if(document.getElementById("f_camas").value != "0"){ // y aun quedan camas compartidas por asignar
						...
						if(com != '0'){ //si la cama esta ocupada para mas adelante de la fecha de llegada
							...
							if(cambio)
								mens="La habitación cambiara de tipo a partir del "+ini+", ¿desea continuar?";
							else
			...
			if(mod != "si" && nume != "0"){ // si se está modificando o no se ha encontrado la solicitud, no se deja asignar camas
				...
				if(asig == 0){	
					...
					if(eval(document.getElementById("f_camas").value) != 0){ //si faltan camas por asinar
						contador=0;
						mens = 0;
						for(i = 1; i <= camas; i++){	// se recorre otra vez todas las celdas de la habitación mirando las que no estan disponibles todos los días
							nom = "td"+hab+"_"+i;
							if(con <= maxi){
								var exi = (document.getElementById(nom)) ? true:false; //dice si existe o no un elemento
								if(exi && document.getElementById(hab+"-"+i).value!='1'){//si existe la celda correspondiente a la cama;
						...
						if(mens == 1){
							alert(contador+" camas no estan disponibles todos los días"); //y se le indica al usuario
		...
		function comprobar_componentes_datos(){
				estado = true;
						...
						if ((document.ingresar_estancia.nombre_componente.value == "") || (document.ingresar_estancia.dni_componente.value == "") || (document.ingresar_estancia.ape1_componente.value == "") ){
							alert("DEBES RELLENAR TODOS LOS CAMPOS");
							estado = false;}
						else{
						
						fecha_naci = new Date(document.ingresar_estancia.annoNa_componente.value,document.ingresar_estancia.mesNa_componente.value,document.ingresar_estancia.diaNa_componente.value);
						fecha_expe = new Date(document.ingresar_estancia.annoEx_componente.value,document.ingresar_estancia.mesEx_componente.value,document.ingresar_estancia.diaEx_componente.value);
					var indice_naci = document.ingresar_estancia.nacionalidad_componente.selectedIndex 
							...
							if(document.ingresar_estancia.nacionalidad_componente[indice_naci].value=="ES"){
									
									if((document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="I")||(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="N")||(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="X")){
									alert("NO PUEDE SER ESE TIPO DE DOCUMENTO CON LA NACIONALIDAD ESPAÑA");
									estado = false;
									}
									if(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="P"){
										if(document.ingresar_estancia.dni_componente.value.length>11){
											alert("NO PUEDE SER EL PASAPORTE MAYOR DE 11 CARACTERES");
											estado = false;
										}
									}
									if((document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="C")||(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="D")){
												var id = document.ingresar_estancia.dni_componente.value;
												...
												if(!isNaN(id.substring(id.length-1,id.length)) ){
														...
														if(isNaN(id.substring(0,id.length-1))){
															...
															if(id.length<8){
																...
																if(id.length == 8){
																	document.ingresar_estancia.dni_modificar.value = "0"+document.ingresar_estancia.dni_modificar.value;
																}}
															}
														}
							...
							if(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="P"){
										if(document.ingresar_estancia.dni_componente.value.length>14){
							...
							if((nacionalidad=="AD")||(nacionalidad=="DE")||(nacionalidad=="AT")||(nacionalidad=="BE")||(nacionalidad=="CY")||(nacionalidad=="DK")||(nacionalidad=="SK")||(nacionalidad=="SI")||(nacionalidad=="EE")||(nacionalidad=="FI")||(nacionalidad=="FR")||(nacionalidad=="GR")||(nacionalidad=="NL")||(nacionalidad=="HU")||(nacionalidad=="IE")||(nacionalidad=="IT")||(nacionalidad=="IS")||(nacionalidad=="UK")||(nacionalidad=="LV")||(nacionalidad=="LT")||(nacionalidad=="LU")||(nacionalidad=="MT")||(nacionalidad=="MC")||(nacionalidad=="NO")||(nacionalidad=="PL")||(nacionalidad=="PT")||(nacionalidad=="SM")||(nacionalidad=="SE")||(nacionalidad=="CH")){
											if((document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="D")||(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="C")||(document.ingresar_estancia.tipo_doc_componente[indice_doc].value=="N")){
											alert("NO PUEDE SER ESE TIPO DE DOCUMENTO CON ESA NACIONALIDAD ");
											estado = false;
											}	}
								else{ 
												if(document.ingresar_estancia.tipo_doc_componente[indice_doc].value!="N"){
												alert("NO PUEDE SER ESE TIPO DE DOCUMENTO CON ESA NACIONALIDAD ");
												estado = false;
												}
	...
	alert("Ya ha dado de alta la reserva del grupo");
		...
		for (i=1; i<=4;i++) {
		 ...
		 if (caja == 3) {
				...
				if ((document.ingresar_estancia.nombreR.value == "") || (document.ingresar_estancia.dniR.value == "") || (document.ingresar_estancia.ape1.value == "")||(document.ingresar_estancia.annoNa.value=="")||(document.ingresar_estancia.annoEx.value=="")||(document.ingresar_estancia.mesEx.value=="")||(document.ingresar_estancia.diaEx.value=="") ){
					alert("Debes rellenar todos los campos");
					estado = false;}
				else{//
				
				fecha_naci = new Date(document.ingresar_estancia.annoNa.value,document.ingresar_estancia.mesNa.value,document.ingresar_estancia.diaNa.value);
				fecha_expe = new Date(document.ingresar_estancia.annoEx.value,document.ingresar_estancia.mesEx.value,document.ingresar_estancia.diaEx.value);
					...
					if(document.ingresar_estancia.telefono.value.length<9){
						alert("El teléfono debe de contar de al menos nueve dígitos");
								estado = false;
					}
				}
...
function valores_negativos(valor){
	...
	//aaaaaaaaaaaaaaaaaaaaaaaaaaaa				
		...
		//aaaaaaaaaaaaaaaaaaaaaaaaaaaa
			 ...
			 if(document.getElementById("nnoches").value < numero_noches){	//si el numero de noches disminuye
 ...
 estado = true;
					...
					if(document.ingresar_estancia.componente_representante.checked==true){
						if(edad_repre<10&&eval(document.getElementById("menos10").value)==0){
							...
							alert("La edad del representante no pertenece a ningún grupo de edad");
							estado = false;
						}else if(edad_repre>=40&&edad_repre<=49&&eval(document.getElementById("entre40_99").value)==0){
							alert("La edad del representante no pertenece a ningún grupo de edad");
							estado = false;
									...
									alert("Tiene que rellenar bien los grupos de edades");
										...
										if(estado){
											...
											if(!(((tem_fechas.length-1) == tem_cuadro.length) || (tem_fechas.length == tem_cuadro.length))){
												alert('Tiene que rellenar la tabla de edades en todos los días que cambien las habitaciones');
												estado = false;
											}
										}
										if(estado){	
			...
			for(i = 0; i < camposTexto.length; i++){	//recorro todos los elementos del formulario
				...
				if(exi && camposTexto[i].type=='hidden' && camposTexto[i].value=='1'){	// y valor el text esta a 1, es que se ha seleccionado esa cama
					...
					if(enc == 0) //si no se encuentra la cama 
						document.getElementById("hab_selec").value = document.getElementById("hab_selec").value + cama + "-"; //lo añado al valor del text que recoge el número de las habitaciones seleccionadas
				}
			}
	...
	function comprobar_componentes_datos2(){
				estado = true;
					...
					var	fecha_naci = new Date(document.ingresar_estancia.annoNa_compo.value,document.ingresar_estancia.mesNa_compo.value,document.ingresar_estancia.diaNa_compo.value);
					
					var	fecha_expe = new Date(document.ingresar_estancia.annoEx_compo.value,document.ingresar_estancia.mesEx_compo.value,document.ingresar_estancia.diaEx_compo.value);
	...
	//funcion para si al poner un pais q no sea españa las provincias no se puedan ingresar
...
var nacionalidad2 = document.ingresar_estancia.pais[indice_naci2].value;
	...
	function comprobar_cif_grupo(){
		...
		if(exi){
			...
			for(i = 0; i < camposTexto.length; i++){
				...
				if(exi && camposTexto[i].type=='hidden' && camposTexto[i].value=='1'){	// si su valor esta a uno, es que se ha seleccionado esa cama
					trozo = camposTexto[i].name.split("-"); // por lo que recogo el número de habitacion
					document.getElementById("hab_selec").value = document.getElementById("hab_selec").value + trozo[0] + "&" + trozo[1] + "-"; // y lo añado al valor de hidden que recoge el número de las habitaciones seleccionadas
					hay = 1;
				}
			}
			if(hay == 1)
	...
	function poner_cambio_hab(fecha, hab, tipo, indi){
		...
		for(i = 0; i < dias_cam.length; i++){	//recorro todas las fechas
			dias_cam1 = dias_cam[i].split("-");
			if(fecha == dias_cam1[1]){			//y si ya existe lo indico
				exi = 1;
			}
		}
		if(exi == 0)	//si la fecha ya esta en el text no se hace nada, y si aun no esta se pone, el guión sirve para separar una fecha de otra
	...
	function quitar_cambio_hab(fecha){
		camposTexto = document.getElementById("ingresar_estancia").elements; //recojo los elementos que hay en el formulario
		enc = 0;	//esta variable dice si hay alguna cama seleccionada con la misma fecha que la que se busca
		for(i1 = 0; i1 < camposTexto.length; i1++){	//recorro todos los elementos del formulario
			trozo = camposTexto[i1].name.split("-"); //por lo que recojo el número de habitacion y de la cama
		...
		if(enc == 0){	//si no se ha encontrado ninguna cama incompleta seleccionada con la misma fecha que la que se buscaba, entonces se quita del text de fecha de las camas incompletas
			...
			for(i2 = 0; i2 < dias_cam.length-1; i2++){	//recorro todas las fechas
				...
				if(fecha != dias_cam1[1]){	//si es distinta a la que se quiere quitar se deja en la cadena
					cad = cad + dias_cam[i2] + "*";
				}
			}
...
<tr> <td width="40%" valign="top">
			 ...
			 <a href="#" onclick="abrir(1)">
			  ...
			  <table border="0" align="center">
			   <tr><td class='label_formulario' >
				...
				<tr>
					<td class='label_formulario'>Dirección:</td><td>
				...
				<tr>
					<td class='label_formulario'>País:</td><td>
				...
				<tr>
					...
					?>
					<option value=''>(Sólo España)</option><?}
	...
	<td class='label_formulario'>
				...
				<tr>
					...
					</select><select name="mesNa" id="mesNa" class="input_formulario" onchange="asignaDias('diaNa','mesNa','annoNa')">
					<?PHP 
					$meses = array("","Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
					if(!$mesNa){ $i=0;
							?>
			 ...
			 <table border="0" align="center">
			   ...
			   <tr>
				...
				<tr>
					...
					?>
					</select><select name="mesNa_componente" id="mesNa_componente" class="input_formulario" onchange="asignaDias('diaNa_componente','mesNa_componente','annoNa_componente')">
					<?PHP 
					$meses = array("","Enero", "Febrero", "Marzo", "Abril", "Mayo", "Junio", "Julio", "Agosto", "Septiembre", "Octubre", "Noviembre", "Diciembre");
   	...
   	// Obtenemos los paises que tenga Carta Europea de la base de datos.
		...
		function comprobar_componentes_datos(){
				estado = true;
						...
						if ((document.ingresar_estancia.nombre_componente.value == "") || (document.ingresar_estancia.dni_componente.value == "") || (document.ingresar_estancia.ape1_componente.value == "")||(document.ingresar_estancia.annoNa_componente.value=="")||(document.ingresar_estancia.annoEx_componente.value=="")||(document.ingresar_estancia.mesEx_componente.value=="")||(document.ingresar_estancia.diaEx_componente.value=="") ){
							alert("Debes rellenar todos los campos");
							estado = false;}
						else{
		...
		?>
		 ...
		 <script>
			...
			<th style="padding:0px 0px 0px 0px;"><a href="?pag=grupos_reserva.php&orden2=DNI&listado_componentes=si&nombre=<?echo $nombre_grupo?>&fecha=<?echo $fecha_llegada?>" style="text-decoration:none"><font color="white">D.N.I.</font></a></th>
											...
											<th><a href="?pag=grupos_reserva.php&orden2=Id_Pais_nacionalidad&listado_componentes=si&nombre=<?echo $nombre_grupo?>&fecha=<?echo $fecha_llegada?>" style="text-decoration:none"><font color="white">País</font></a></th>
...
?>
		...
		<input name="hab_selec" id="hab_selec" type="text" value="<?=$_POST['hab_selec']?>">	<!-- guarda las camas seleccionadas de los días que se han visto -->
...
if(!isset($_POST['no_leer'])){
	...
	$fec_selec = $fec_selec . $crear_fechas[0] ."+";
	$hab_selec_tem = split("\*",$crear_hab[0]);
		for($h = 0; $h < count($hab_selec_tem); $h++){
			$hab_cama = split("-",$hab_selec_tem[$h]);
			for($x = 1; $x <= $hab_cama[1]; $x++){
				$hab_selec = $hab_selec . $hab_cama[0] . "&" . $x . "-";
			}
		}
	$hab_selec = $hab_selec . "*";
	
	echo "<script>document.getElementById('hab_selec').value='" . $hab_selec . "'</script>";
	echo "<script>document.getElementById('fec_selec').value='" . $fec_selec . "'</script>";
	$_POST['fec_selec'] = $fec_selec;
	$_POST['hab_selec'] = $hab_selec;
	$_POST['fecha_ver'] = $crear_fechas[0];
	$_POST['fecha_cal'] = substr($crear_fechas[0],0,2)."-".substr($crear_fechas[0],3,2)."-".substr($crear_fechas[0],6,4);
	$_POST["mov_cal"]='1';
...
?>		
		...
		?>				
				...
				else{ //si todas las camas de la habitación estan ocupada o ya están en asignadas se indica que falta asignar una cama
					enc = 0;
					i = 0;
...
$result=mysql_query($sql_comprobar_grupo);
	...
	if(mysql_num_rows($result)!=0){
		echo "<script>alert('Este grupo ya está realizando una pernocta');</script>";
		echo "<script>document.getElementById('comprobar_grupo').value='no'</script>";
