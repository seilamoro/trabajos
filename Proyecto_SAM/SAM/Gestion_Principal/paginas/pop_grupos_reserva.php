	...
	function llevar() {
		...
		for (i=0;i<document.getElementsByName('op').length;i++){
			if (document.getElementsByName('op')[i].checked == true) {
				...
				window.opener.document.ingresar_estancia.cif.value = array_cadena[8];
				
				window.opener.document.ingresar_estancia.nombreG.value = array_cadena[0];
				window.opener.document.ingresar_estancia.dir.value = array_cadena[1];
				
				window.opener.document.ingresar_estancia.localidad.value = array_cadena[2];
				...
				//window.opener.document.dar_alta.prov.options[array_cadena[4]].selected = true;
					...
					for(j=0;j<window.opener.document.ingresar_estancia.prov.length;j++)	{
						if(j == provincia)	{
							window.opener.document.ingresar_estancia.prov.options[j].selected = true;
							break;
						}
						
					}
					if(array_cadena[7]=="xxx"){
				window.opener.document.ingresar_estancia.prov.disabled=true;
				}
					var pais = array_cadena[3];
					
					for(j=0;j<window.opener.document.ingresar_estancia.pais.length;j++)	{
						if(j == pais)	{
							window.opener.document.ingresar_estancia.pais.options[j-1].selected = true;
							break;
						}
						
					}
					window.opener.document.ingresar_estancia.email.value = array_cadena[6];
				
				
				window.opener.document.ingresar_estancia.con_popup.value = 1;
...
<form name="formu" action="pop_grupos_2.php" method="POST">
	...
	<table border="0" width="575px" align="left">
		<tr align="left" >
			<td></td>
			<td >CIF: </td>
			<td >Nombre: </td>
...
@ $db = mysql_pconnect($_SESSION['conexion']['host'],$_SESSION['conexion']['user'],$_SESSION['conexion']['pass']);
			...
			mysql_select_db($_SESSION['conexion']['db']);
					...
					$result2=mysql_query("select DISTINCT * from grupo where grupo.Nombre_Gr LIKE '".$_POST['criterio_nombre']."%' and grupo.Localidad_Gr LIKE '".$_POST['criterio_localidad']."%' and grupo.Id_Pais LIKE '".$_POST['criterio_pais']."%'");
...
//echo "select DISTINCT * from grupo,estancia_gr $where GROUP BY grupo.Nombre_Gr";
			...
			while ($row=mysql_fetch_array($result2)) {
				$result_pais=mysql_query("select * from pais");
				$i=0;
				...
				$result_pro=mysql_query("select * from provincia");
				$j=0;
				...
				$sql="select DISTINCT * from grupo,estancia_gr where grupo.Nombre_Gr=estancia_gr.Nombre_Gr  and grupo.Nombre_Gr='".$row["Nombre_Gr"]."' and Fecha_Salida>='".date("Y-m-d")."'";
					 ...
					 $result_incidencia=mysql_query("select * from incidencias where Nombre_Inc='".$row["Nombre_Gr"]."'");

			?>
			<tbody style="overflow:auto;font-weight: normal;">
				<?  if(mysql_num_rows($result)==0){ ?>
			<tr align="center" ><td width="3%">
			<?
			...
			?>
			<input type="radio" name="op" value="<?echo $row["Nombre_Gr"]?>#<?echo $row["Direccion_Gr"]?>#<?echo $row["Localidad_Gr"]?>#<?echo $offset?>#<?echo $offset_pro?>#<?echo $row["Id_Pais"]#<? echo "ccddddd"; ?>#<?echo $row["Email_Gr"]?>#xxx#<?echo $row["CIF"]?>">
			<?}
			?>
			</td>
					<?
			...
			$nombre_gr_partido = parteCadenas($nombre_gr, 28);
			...
			$result_provin=mysql_query("select * from provincia where id_Pro ='".$row["Id_Pro"]."'");
			$row_provin=mysql_fetch_array($result_provin);
									$loca = $row['Localidad_Gr'];
			$loca_partido = parteCadenas($loca, 17);
			echo "<td valign='top' width='20%' align='left' class='discrete_text'><font color='gray'>".$loca_partido."</font></td>";
