<script language="JavaScript">
	...
	function exentos_facturados(num_personas,campo_exentos,campo_facturados,clave,fecha) {
	  	...
	  	formulario.gdh_dis.value = 'FG';
	  	formulario.gdh_fac.value = 'G' + '*' + clave + '*' + fecha;
	  	formulario.submit();
	}
	
	//Funcion que cambia la cantidad al modificar el dinero de desperfectos
	function modificar_total_factura(resto,campo_total,desperfecto) {
	  	if (desperfecto.value == '') {
		  	campo_total.value = parseFloat(resto) + ' €';
		}
		else if (!isNaN(parseFloat(desperfecto.value))) {
		  	campo_total.value = (parseFloat(resto) + parseFloat(desperfecto.value)) + ' €';
		}
	}
...
?>			
											  		...
											  		<td align="right">		  		
											  			<input type=text size=2 class="input_text_numerico" name="facturados_<?PHP echo $pernocta_gr_individual['Id_Hab'].'_'.$pernocta_gr_individual['Id_Edad']; ?>" value="<?PHP echo $_SESSION['factura_grupo']['Num_Facturados'][$i]; ?>">
											  		</td>
											    </tr>
<?PHP
		}
...
<?PHP
		$resto = FLOATVAL($fila['PerNocta'] * ($dinero_noche + $plus) - $fila['Ingreso']).' €';
?>
				<tr>												
					<td class="texto_detalles" align="left">
						Cuantía:
...
?>
						<input type="text" name="cuantia_desperfecto_factura" size=6 maxlength=7 contenteditable=true class="input_text_vacio_numerico" value="<?PHP echo $_POST['cuantia_desperfecto_factura']; ?>" onkeyup="modificar_total_factura('<?PHP echo $resto; ?>',total_importe_factura,cuantia_desperfecto_factura);">
<?PHP
		}
		else {
		  	$desperfecto = 0;
?>
						<input type="text" name="cuantia_desperfecto_factura" size=6 maxlength=7 contenteditable=true class="input_text_vacio_numerico" value="0.0" onkeyup="modificar_total_factura('<?PHP echo $resto; ?>',total_importe_factura,cuantia_desperfecto_factura);">
<?PHP
		}
?>
					</td>												
				...
				<tr>
					...
					<td align="left">
						...
						<input type="text" name="total_importe_factura" size=9 contenteditable=false style="border-style:2px solid;border-color:#064C87;background-color:<?PHP echo $_SESSION['gdh']['colores']['normal']; ?>;color:#064C87;font-size:15px;font-weight:bold;text-align:right;" value="<?PHP echo ($desperfecto + $resto).' €'; ?>">
					</td>
				</tr>
				<tr>
...
?>					
									<img src="./../imagenes/botones-texto/facturar.jpg" class="imagen-boton" title="Crear Nueva Factura" value="Facturar" onclick="comprueba_dni('FG');">
<script language="javascript">
	...
	function comprueba_dni(tipo) {
		...
		if (resultado == true) {
			...
			alert("El D.N.I. no es válido.");
									...
									<input type="hidden" name="ingreso_factura" value="<?PHP echo $fila['Ingreso']; ?>">
								</td>
							</tr>
						</table>
					</td>
