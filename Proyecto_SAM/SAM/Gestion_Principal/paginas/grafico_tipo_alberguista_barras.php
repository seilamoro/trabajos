...
//Fin del título
	...
	$datay = array($alberguistas, $personas_grupos);
	$datay2 = array(0,0);
	if ($datay == $datay2){
	/***************************** INICIO NO HAY DATOS *********************************/
	...
	$graph->img->SetMargin(40,40,65,30); 
	...
	$graph->SetShadow('darkgray@0.5');
	$bplot->SetFillColor('lightblue');
	$bplot->SetWidth(0.5);
	$bplot->SetShadow('darkgray@0.5');
	
	$bplot->value->Show();
	$bplot->value->SetFont(FF_FONT2,FS_NORMAL);
	...
	//$graph->xaxis->title->Set("Tipo de Alberguista");
	//$graph->xaxis->title->SetColor("#ffffff");
	
	//$graph->yaxis->title->Set("Número");
	$graph->yaxis->title->SetColor("black");
	$graph->SetFrame(true,'black','white@0.4','white@0.4',4,2);
	$graph->SetColor("#f4fcff");
	$graph->subtitle->SetFont(FF_FONT2,FS_NORMAL);
	$graph->xaxis->SetFont(FF_FONT2,FS_NORMAL); 

	$entrada = array("Individual", "Grupos (".$numero_grupos.")");
	
	$graph->xaxis->SetTickLabels($entrada);
	
	$graph->yaxis->title->SetFont(FF_FONT2,FS_BOLD);
	$graph->yaxis->title->SetMargin(8,0,0,0);
	$graph->yaxis->SetColor("black");
	$graph->xaxis->title->SetFont(FF_FONT2,FS_BOLD);
	$graph->xaxis->title->SetMargin(8,0,0,0);
