...
public class TimestampVector implements Serializable{
	...
	public void updateTimestamp(Timestamp timestamp){
		if(timestamp == null)
			return;
		String opHost = timestamp.getHostid(); //Get the user
		timestampVector.replace(opHost, timestamp); //Update the timestamp user in vector
	}
	...
	public void updateMax(TimestampVector tsVector){
		...
		for (String host : this.timestampVector.keySet()) {
            ...
            if(hostTimestamp != null && this.getLast(host).compare(hostTimestamp) < 0){
	...
	public void mergeMin(TimestampVector tsVector){
		...
		for (ConcurrentHashMap.Entry<String, Timestamp> entry : tsVector.timestampVector.entrySet()) {
            String host = entry.getKey();
            Timestamp otherTimestamp = entry.getValue();
            Timestamp hostTimestamp = this.getLast(host);
            
            if (hostTimestamp == null) {
                this.timestampVector.put(host, otherTimestamp);
            } else if (otherTimestamp.compare(hostTimestamp) < 0) {
                this.timestampVector.replace(host, otherTimestamp);
            }
        }
	}
	...
	public boolean equals(Object obj){
		...
		TimestampVector objTimestampVector = (TimestampVector) obj;
		return objTimestampVector.timestampVector.equals(this.timestampVector);
	}
