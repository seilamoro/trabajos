	...
	public void run() {
		...
		try {
			...
			if (msg.type() == MsgType.AE_REQUEST){
	            ...
	            // send operations
				 for (Operation op : serverData.getLog().listNewer(msgRequest.getSummary())) {
				...
				while (msg.type() == MsgType.OPERATION){
					synchronized (serverData) { 
						serverData.messageOperationProcess((MessageOperation)msg);
					} 
					msg = (Message) in.readObject();
					lsim.log(Level.TRACE, "[TSAESessionPartnerSide] [session: "+current_session_number+"] received message: "+ msg);
				}
				...
				if (msg.type() == MsgType.END_TSAE){
		            ...
		            out.writeObject(msg);					
					lsim.log(Level.TRACE, "[TSAESessionPartnerSide] [session: "+current_session_number+"] sent message: "+ msg);
					synchronized (serverData) { ////update summaries and trigger the message ordering component
						serverData.getSummary().updateMax(msgRequest.getSummary());
						serverData.getAck().updateMax(msgRequest.getAck());
