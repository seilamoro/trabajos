...
public class ServerData {
	...
	public synchronized void removeRecipe(String recipeTitle){
		Timestamp timestamp = nextTimestamp();
		Operation op = new RemoveOperation(recipeTitle, this.recipes.get(recipeTitle).getTimestamp(), timestamp);
	...
	public synchronized void messageOperationProcess(MessageOperation msg){
		if(msg == null)
			return;
		
		Operation op = msg.getOperation(); //--
		if(this.getLog().add(op)) {
			if(op.getType() == OperationType.ADD){
				AddOperation add = (AddOperation)op;
				this.recipes.add(add.getRecipe());
			} 
			else if (op.getType() == OperationType.REMOVE){
				RemoveOperation remove = (RemoveOperation)op;
				this.recipes.remove(remove.getRecipeTitle());
			}
		}
	}
}