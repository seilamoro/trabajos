...
$conf['database']['campos_sql'][0]['nombre_mostrar'] = 'Parámetro';
...
$conf['texto']['titulo_tabla'] = 'Listado de Parámetros del Centro';
...
//Coger la última coma y la sustituya por un blanco para hacer la consulta sql...
...
if ($_GET['listado_accion'] == 'new') {
    ...
    if($hay>=1 or $sonda_nueva==""){
      echo "<script>alert('Esa sonda ya tiene un parámetro asociado');</script>";
    }
    else{
...
<script language="JavaScript">
	...
	// Función que asigna el foco al cuadro de texto que se modifico por última vez
	function foco(i,s,e)	{
	...
	// Función que recarga la página con el fin de ordenar por algún campo las filas.
					...
					<?PHP
						$db_link = MYSQL_CONNECT($conf['database']['host'],$conf['database']['user'],$conf['database']['pass']);
							...
							if($i==0){
                                ...
                                <?PHP
                                $p=1;
                                print ("<option value='s'>-- Seleccione Parámetro --</option>");
