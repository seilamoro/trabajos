...
$conf['general']['nombre_pagina'] = 'listado_parametros.php';
...
$conf['database']['nombre_tabla'] = 'parametro';
$conf['database']['clave_tabla'][0] = 'Id_Parametro';
...
$conf['database']['campos_sql'][0]['logitud_max'] = '30';
...
$conf['database']['campos_sql'][1]['nombre_db'] = 'Descripcion_Param';
$conf['database']['campos_sql'][1]['nombre_mostrar'] = 'Descripción';
$conf['database']['campos_sql'][1]['logitud_max'] = '150';
$conf['database']['campos_sql'][1]['nombre_intext'] = 'intext_'.$conf['database']['campos_sql'][1]['nombre_db'];
$conf['database']['campos_sql'][1]['logitud_intext'] ='70';
...
$conf['texto']['titulo_tabla'] = 'Listado de Parámetros';
...
//$conf['menu_contextual'][2]['link'] = 'listado_parametros.php';
...
<form name="otro_form" action="?pag=listado_parametros.php">
<input type="hidden" name="sielimino"  id="sielimino"  value="" >
</form>
...
if($_GET['sielimino'])
{
		...
		if (!$db_link) {
			...
			$query="delete from parametro where Id_Parametro = ";
			$j = COUNT($_SESSION['param_seleccionados']);
			for ($i = 0; $i < $j; $i++)
			{
				if($i==0)
				{
					$query .= $_SESSION['param_seleccionados'][$i];
				}
				else
				{
					$query .= " OR Id_Parametro = " .$_SESSION['param_seleccionados'][$i];
				}
			}
...
//	echo $sql_ini;
	...
	?>
	<script>
        location.href='?pag=listado_parametros.php';
    </script>
<?php }
...
if ($_GET['confirmar_eliminar'] != '') {
	if ($_GET['confirmar_eliminar'] == 'rs') {
		for ($k = 0; $k < count($_GET['checks']); $k++){
            $sql='SELECT * FROM '.$conf['database']['nombre_tabla'].', sonda WHERE sonda.id_parametro= ';
			$sql = $sql.'"'.$_GET['checks'][$k].'" AND '.$conf['database']['nombre_tabla'].'.id_parametro="'.$_GET['checks'][$k].'"';
            ...
            $_SESSION['param_seleccionados'][$k]=$_GET['checks'][$k];
            
    		if($filas>=1){?>

                <script language='javascript'>
					var respuesta=confirm("Alguno de los parametros tienen sonda asociada. ¿Desea continuar?");
					if(!respuesta)
					{
                        document.getElementById('sielimino').value=0;
                        location.href='?pag=listado_parametros.php&sielimino=0';
					}
					else
					{
                        document.getElementById('sielimino').value=1;
                        location.href='?pag=listado_parametros.php&sielimino=1&id_eliminar=<?echo $_GET['checks'][$k]?>';

					}
                </script>

            <?php } //Cierro el if de la comprobacion num filas
            else{
        			$sql = 'DELETE FROM '.$conf['database']['nombre_tabla'].' WHERE ';
        			$tope = count($conf['database']['clave_tabla']);
        			$aux = split('\\'.$conf['database']['separador_clave'],$_GET['checks'][$i]);
        			for ($z = 0; $z < $tope; $z++) {
        				$sql = $sql.$conf['database']['clave_tabla'][$z].' = "'.$_GET['checks'][$k].'" AND ';
        			}
        			$sql = SUBSTR($sql,0,-5).';';
        			$db_link = MYSQL_CONNECT($conf['database']['host'],$conf['database']['user'],$conf['database']['pass']);
        			if (!$db_link) {
		...
		for ($i = 0; $i < $tope; $i++) {
			$sql = $sql.'"'.$aux[$i].'" AND '.$conf['database']['nombre_tabla'].'.id_parametro="'.$aux[$i].'"';
		}
        $db_link = MYSQL_CONNECT($conf['database']['host'],$conf['database']['user'],$conf['database']['pass']);
		if (!$db_link) {
		   die("Could not connect: ".MYSQL_ERROR());
...
<form name="<?PHP echo $conf['general']['nombre_formulario']; ?>" action="?pag=listado_parametros.php">

<input type="hidden" name="listado_accion" value="">
<input type="hidden" name="pag" value="<?PHP echo $conf['general']['nombre_pagina'] = 'listado_parametros.php'; ?>">
...
?>
				<tr id="<?PHP echo $fila['Clave']; ?>" class="accion">
					...
					<td class="accion">
					<input type="hidden" name="<?PHP echo 'm'.$conf['database']['campos_sql'][0]['nombre_intext']; ?>" size="<?PHP echo $conf['database']['campos_sql'][0]['logitud_intext']; ?>" maxLength="<?PHP echo $conf['database']['campos_sql'][0]['logitud_max']; ?>" value="<?PHP echo $fila[$conf['database']['campos_sql'][0]['nombre_db']]; ?>">
						<div class="intexto">
							<?PHP echo $fila[$conf['database']['campos_sql'][0]['nombre_db']]; ?>
						</div>
					</td>
<?PHP	
