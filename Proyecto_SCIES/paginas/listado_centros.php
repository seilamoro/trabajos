...
$conf['database']['campos_sql'][1]['nombre_mostrar'] = 'Dirección';
...
$conf['database']['campos_sql'][2]['nombre_mostrar'] = 'Teléfono';
...
$conf['database']['campos_sql'][4]['nombre_mostrar'] = 'Todo<br>el Año';
...
$conf['texto']['todo_anio'] = 'Todo el<br> Año';
...
$conf['menu_contextual'][2]['texto'] = 'Parámetros';
...
//Campos que añado para hacer el select
	  ...
	  var diciem="31-12-"+anioactual;
		...
		if((nombre=='') || (dire=='') || (telef=='') || (local=='')) 
		 ...
		 if (telef.length>9 || isNaN(telef) || telef.length<9)
			{
		  		alert("Debe introducir un formato válido en el campo Teléfono");
		  		return false;
	   	...
	   	if(!document.formulario_listado.seleccion.checked) {
	 			...
	 			// No está pulsado el botón para añadir meses
...
?>
					<a href="#" align=center class="volver">
						...
						<div class="volver" title="Página Aterior" onclick="history.back();">
						</div>
...
?>					
					...
					<a href="#" class="confirmar_aceptar">
						<td width='25' onclick="confirmarModificar('<?PHP echo $conf['menu_valor']; ?>',<?print($link)?>);" title="Aceptar Modificación">
						</td>
					</a>
	  	...
	  	//alert(document.formulario_listado.seleccionm.checked);
				...
				if (clave != 'NULL') {
						...
						if(document.formulario_listado.seleccionm.checked == false) {
	  					...
	  					window.open('paginas/fechas_operativas.php?accion=m'+link,'ventana1','width=400px,height=175px,resizable=yes,toolbar=no,channelmode=no,directories=no,location=no,status=no,titlebar=no,top=px,left=550px')
	  							}  else {//hay que cambiar a todo el año
